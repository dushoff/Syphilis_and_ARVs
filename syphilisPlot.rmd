...
First row and firt column show epidemic curves with only HIV and syphilis, respectively. 3 by 3 plots show epidemic curves when both diseases are present.
...
prev_name[["co"]] <- c("N","I.HIV","T.HIV","I.syph","T.syph")
...
g.HIV <- ggplot(sumFrame[["HIV"]], aes(time, value, col = .id)) +
	geom_line() +
	theme(legend.position="none",
		panel.margin = unit(1, "lines"),
...
g.syph <- ggplot(sumFrame[["syph"]], aes(time, value, col = .id)) +
	geom_line() +
	theme(legend.position="none",
		panel.margin = unit(1, "lines")) +
	labs(x = NULL, y = NULL) +
...
ggplot(subset(sumFrame[["co"]], L1 %in% prev_name[["co"]][-1]), aes(time, value, col = .id)) +
	geom_line() +
	facet_wrap(~L1, scale = "free")
